<template>
  <div>
      ...
      <div class="field">
    <h1 class="title has-text-centered">Ausgabe bearbeiten</h1>
    </div>
...
export default {
    ...
    methods: {
        async load() {
            ...
            if (r.ok && r.content) {
                this.mitreisende = r.content.mitreisende;
                let elem = r.content.belege.filter( x => x.id == this.$route.params.payment)[0]
                this.value = elem.betrag / 100;
                this.name = elem.name;
                this.by = elem.von.mail;
                this.für = elem.an.map(x => {return x.mail});
                this.time = elem.datum;
            }
            if (r.status == 401) this.$router.push("/login?login=true")
        },
        async save() {
            //TODO: Error Handeling
            let r = await api.PUT("reisen/" + encodeURIComponent(this.$route.params.id) + "/beleg/" + encodeURIComponent(this.$route.params.payment), {
                "name": this.name,
                "datum": this.time,
